...
var live = {
	//!steal-remove-start
	// maps DOM elements to the compute they are bound to
	_elementDependenciesMap: new WeakMap(),
	//!steal-remove-end
	...
	listen: function(el, compute, change) {
		return live.setup(
			...
			function bind() {
				...
				//!steal-remove-start
				var deps = live._elementDependenciesMap.get(el);
				if (deps) {
					deps.add(compute);
				} else {
					live._elementDependenciesMap.set(el, new Set([compute]));
				}
				//!steal-remove-end
			},
			function unbind(data) {
				...
				deps.delete(compute);
				if (!deps.size) {
					live._elementDependenciesMap.delete(el);
				}
				//!steal-remove-end
...
Element.prototype[getValueDependenciesSymbol] = function getValueDependencies() {
	var el = this;
	if (live._elementDependenciesMap.has(el)) {
		return {
			valueDependencies: live._elementDependenciesMap.get(el)
		};
	}
};
//!steal-remove-end
