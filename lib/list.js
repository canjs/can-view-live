...
var renderAndAddRangeNode = function(render, context, args) {
...
ListDOMPatcher.prototype = {
	...
	add: function(items, index) {
		...
		var frag = this.range.start.ownerDocument.createDocumentFragment(),
			newEndNodes = [],
			newIndicies = [],
			render = this.render,
		...
		items.forEach( function(item, key) {
			...
			var itemIndex = new SimpleObservable(key + index),
				...
				itemFrag = renderAndAddRangeNode(render, context, [itemCompute, itemIndex]);
	...
	addFalseyIfEmpty: function() {
		if (this.falseyRender && this.indexMap.length === 0) {
			...
			var falseyFrag = renderAndAddRangeNode(this.falseyRender, this.currentList, [this.currentList]);
 ...
 *
 */
live.list = function(el, list, render, context, falseyRender) {
