...
function ListDOMPatcher(el, compute, render, context, parentNode, nodeList, falseyRender) {
	...
	this.onPatchesUpdate = this.onPatchesUpdate.bind(this);

	var stopTrap = this.patcher.trapPatchBindings();
	...
	//!steal-remove-start
	Object.defineProperty(this.onPatchesUpdate, "name", {
...
ListDOMPatcher.prototype = {
	setupValueBinding: function() {
		this.patcher[onPatchesSymbol](this.onPatches, "derive");

		if (this.patcher.currentList && this.patcher.currentList.length) {
			this.onPatchesUpdate([{
	...
	teardownValueBinding: function() {
		this.patcher[offPatchesSymbol](this.onPatches, "derive");
		this.exit = true;
		this.remove({
	...
	onPatches: function ListDOMPatcher_onPatches(patches) {
		//this.onPatchesUpdate(patches);
		//return;
		queues.domUIQueue.enqueue(
			this.onPatchesUpdate,
			this,
			[patches],
			{
				...
				, log: [ canReflect.getName(this.onPatchesUpdate) ]
	...
	onPatchesUpdate: function ListDOMPatcher_onPatchesUpdate(patches) {
		if (this.exit) {
			return;
		}
