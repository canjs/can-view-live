...
ListDOMPatcher.prototype = {
	setupValueBinding: function () {
		...
		this.onPatches([{insert: this.patcher.currentList, index: 0, deleteCount: 0}]);
	},
	teardownValueBinding: function(){
		this.patcher[offPatchesSymbol](this.onPatches,"notify");
		this.exit = true;
		this.onPatches([{deleteCount: this.patcher.currentList.length, index: 0}]);
	},
		...
		each(items, function (item, key) {
			...
			var itemIndex = new SimpleObservable(key + index),
				itemCompute = new SetObservable(item, function(newVal){
					canReflect.updateList(this.currentList, newVal);
				}.bind(this)),
	...
	addFalseyIfEmpty: function(){
		if(this.falseyRender && this.currentList.length === 0){
	...
	move: function move( item, newIndex, currentIndex ) {
		//if (!afterPreviousEvents) {
		//	return;
		//}
...
live.list = function (el, compute, render, context, parentNode, nodeList, falseyRender) {
	if(el.nodeType !== Node.TEXT_NODE) {
		throw new Error("can-view-live.list only works on text nodes now");
	}
	new ListDOMPatcher(el, compute, render, context, parentNode, nodeList, falseyRender)
};
