...
// Used to hide the methods to add/remove the DOM element dependencies
// from the _elementDependenciesMap from the consumers of ListDOMPatcher
...
function ListDOMPatcher(el, compute, render, context, parentNode, nodeList, falseyRender) {
	...
	//!steal-remove-start
	// keep logic to register `can.getValueDependencies` private
	privates.set(this, {
		addValueDependency: function addValueDependency() {
			live._elementDependenciesMap.set(parentNode, new Set([compute]));
		},
		deleteValueDependency: function deleteValueDependency() {
			live._elementDependenciesMap.delete(parentNode);
		}
	});
	//!steal-remove-end
...
ListDOMPatcher.prototype = {
	setupValueBinding: function() {
		...
		//!steal-remove-start
		privates.get(this).addValueDependency();
		//!steal-remove-end
	},
	teardownValueBinding: function() {
		...
		//!steal-remove-start
		privates.get(this).deleteValueDependency();
		//!steal-remove-end
	},
