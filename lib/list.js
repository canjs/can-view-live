...
live.list = function (el, compute, render, context, parentNode, nodeList, falseyRender) {
	...
	var masterNodeList = nodeList || [el],
		...
		// The current list.
		list,
		// a queue for `add`, `set`, `remove`, and `move`
		updateQueue = [],
		...
		teardownList = function (fullTeardown) {
			...
			if (list && list.removeEventListener) {
				list.removeEventListener('add', queueAdd);
				list.removeEventListener('set', queueSet);
				list.removeEventListener('remove', queueRemove);
				list.removeEventListener('move', queueMove);
			}
		...
		oldList,
		// wrap `fn` in `Observation.registerUpdate`
		...
		// queueAdd that will wrap `add` in an Observation.registerUpdate
		queueAdd = queueFn(add),
		// queueSet that will wrap `set` in an Observation.registerUpdate
		queueSet = queueFn(set),
		// queueRemove that will wrap `remove` in an Observation.registerUpdate
		queueRemove = queueFn(remove),
		// queueMove that will wrap `move` in an Observation.registerUpdate
		queueMove = queueFn(move),
		...
		updateList = function (newList) {
			...
			if(newList && oldList) {
				...
				if ( oldList.removeEventListener ) {
					oldList.removeEventListener('add', queueAdd);
					oldList.removeEventListener('set', queueSet);
					oldList.removeEventListener('remove', queueRemove);
					oldList.removeEventListener('move', queueMove);
				}
			...
			if (list.addEventListener) {
				...
				list.addEventListener('add', queueAdd);
				list.addEventListener('set', queueSet);
				list.addEventListener('remove', queueRemove);
				list.addEventListener('move', queueMove);
			}
