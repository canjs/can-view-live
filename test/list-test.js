...
QUnit.test("basics", function (assert) {
	var div = document.createElement("div"),
		list = new DefineList(["sloth", "bear"]),
	...
	assert.equal(
		div.getElementsByTagName("label").length,
		2,
		"There are 2 labels"
	);
	...
	list.push("turtle");
	assert.equal(
		div.getElementsByTagName("label")[0].myexpando,
		"EXPANDO-ED",
		"same expando"
	);
	assert.equal(
		div.getElementsByTagName("span")[2].innerHTML,
		"turtle",
		"turtle added"
	);
});
...
QUnit.test("list within an Observation", function (assert) {
	...
	var div = document.createElement("div"),
		map = new SimpleMap({
			animals: new DefineList(["bear", "turtle"]),
		}),
		template = function (animal) {
			return "<label>Animal=</label> <span>" + animal.get() + "</span>";
		};
	...
	map.attr("animals", new DefineList(["sloth", "bear", "turtle"]));
...
QUnit.test(
	".list within a observable value holding an Array list",
	function (assert) {
		var div = document.createElement("div");
...
QUnit.test("live.list should handle move patches", function (assert) {
	...
	*/
	...
	list.splice(1, 0, "a");
	queues.batch.start();
	onPatchesHandler([{ type: "move", fromIndex: 0, toIndex: 1 }]);
	queues.batch.stop();
...
QUnit.test(
	"list items should be correct even if renderer flushes batch (#8)",
	function (assert) {
		...
		assert.equal(
			partial.getElementsByTagName("span").length,
			2,
			"should be two items"
		);
		assert.equal(
			partial.getElementsByTagName("span")[0].firstChild.data,
			"one",
			'list item 0 is "one"'
		);
		assert.equal(
			partial.getElementsByTagName("span")[1].firstChild.data,
			"two",
			'list item 1 is "two"'
		);
...
QUnit.test(
	"can insert at multiple list indexes when flusing batch (#150)",
	function (assert) {
		var partial = document.createElement("div");
		var placeholderElement = document.createElement("span");
		var list = new DefineList(["one", "two"]);
		...
		assert.equal(
			partial.getElementsByTagName("span")[2].firstChild.data,
			"one and a half",
			'list item 1 is "one"'
		);
		assert.equal(
			partial.getElementsByTagName("span")[3].firstChild.data,
			"two",
			'list item 0 is "two"'
		);
		assert.equal(
			partial.getElementsByTagName("span")[4].firstChild.data,
			"three",
			'list item 1 is "three"'
		);
	}
);

QUnit.test(
	"can remove and insert at multiple list indexes when flusing batch",
	function (assert) {
		var partial = document.createElement("div");
		var placeholderElement = document.createElement("span");
		var list = new DefineList(["one", "two"]);
...
testHelpers.dev.devOnlyTest("can-reflect-dependencies", function (assert) {
	...
	assert.deepEqual(
		canReflectDeps.getDependencyDataOf(placeholder).whatChangesMe.mutate
			.valueDependencies,
		new Set([list])
	);
